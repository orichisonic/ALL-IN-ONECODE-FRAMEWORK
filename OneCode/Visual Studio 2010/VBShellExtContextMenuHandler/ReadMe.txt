=============================================================================
      CLASS LIBRARY : VBShellExtContextMenuHandler Project Overview
=============================================================================

/////////////////////////////////////////////////////////////////////////////
Summary:

The VB.NET code sample demonstrates creating a Shell context menu handler 
...
on the machine. VBShellExtContextMenuHandler is such a managed shell extension 
...
It adds the menu item "Display File Name (VB)" to the context menu when you 
right-click a .vb file in the Windows Explorer. Clicking the menu item 
brings up a message box that displays the full path of the .vb file.
...
Install VBShellExtContextMenuHandlerSetup(x86).msi, the output of the 
VBShellExtContextMenuHandlerSetup(x86) setup project, on a x86 operating 
system. If the target operating system is x64, install 
VBShellExtContextMenuHandlerSetup(x64).msi outputted by the 
VBShellExtContextMenuHandlerSetup(x64) setup project.

B. Removal

Uninstall VBShellExtContextMenuHandlerSetup(x86).msi, the output of the 
VBShellExtContextMenuHandlerSetup(x86) setup project, on a x86 operating 
...
you will get a DLL: VBShellExtContextMenuHandler.dll. Run 'Visual Studio 
...
contains the build result VBShellExtContextMenuHandler.dll and enter the 
command:
...
Step2. Find a .vb file in the Windows Explorer (e.g. FileContextMenuExt.vb in 
the sample folder), and right click it. You would see the "Display File Name 
(VB)" menu item in the context menu and a menu seperator below it. Clicking 
the menu item brings up a message box that displays the full path of the .vb 
file.

The "Display File Name (VB)" menu item is added and displayed when only one 
.vb file is selected and right-clicked. If more than one file are selected 
...
project named "VBShellExtContextMenuHandler". Open the project properties, 
and in the Signing page, sign the assembly with a strong name key file. 
...
The FileContextMenuExt.vb file defines a context menu handler. The context 
menu handler must implement the IShellExtInit and IContextMenu interfaces. 
The interfaces are imported using the COMImport attribute in ShellExtLib.vb. 
...
registered.
  ...
  In this code sample, we insert the menu item "Display File Name (VB)", and 
...
ShellExtReg.RegisterShellExtContextMenuHandler method in ShellExtLib.vb to 
...
For example, this code sample associates the handler with '.vb' files. 
HKCR\.vb has the default value 'VisualStudio.vb.10.0' by default when 
Visual Studio 2010 is installed, so we proceed to register the handler under 
HKCR\VisualStudio.vb.10.0\ instead of under HKCR\.vb. The following keys 
    ...
    HKCR
    {
        ...
        NoRemove VisualStudio.vb.10.0
        {
            NoRemove shellex
            {
                ...
                {
                    {1E25BCD5-F299-496A-911D-51FB901F7F40} = 
                        s 'VBShellExtContextMenuHandler.FileContextMenuExt Class'
                }
            }
        }
    }
...
D. Deploying the context menu handler with a setup project.
  ...
  (1) In VBShellExtContextMenuHandler, add an Installer class (named 
  ...
  VBShellExtContextMenuHandlerSetup(x86). Click OK to create the project. 
  ...
  In the Add Project Output Group dialog box, VBShellExtContextMenuHandler 
  ...
  VBShellExtContextMenuHandler. This adds the custom actions that we defined 
  in ProjectInstaller of VBShellExtContextMenuHandler. 
  ...
  Framework version of VBShellExtContextMenuHandler.
  ...
  must create a new setup project (e.g. VBShellExtContextMenuHandlerSetup(x64) 
  ...
  file in the VBShellExtContextMenuHandlerSetup(x64) project folder. To 
  configure the script to run in the post-build event, you select the 
  VBShellExtContextMenuHandlerSetup(x64) project in Solution Explorer, and 
