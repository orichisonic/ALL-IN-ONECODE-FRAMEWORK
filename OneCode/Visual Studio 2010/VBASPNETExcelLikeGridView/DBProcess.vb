...
Public NotInheritable Class DBProcess
    ...
    ''' </summary>
    Public Function GetDataTable(ByVal create As Boolean) As DataTable
        If create Then
            Using fs As New FileStream("Table.dat", FileMode.Create)
                dt = New DataTable()
                adapter.Fill(dt)
            ...
            End Using
        Else
            Using fs As New FileStream("Table.dat", FileMode.Open)
                Dim bf As New BinaryFormatter()
                dt = DirectCast(bf.Deserialize(fs), DataTable)
            End Using
        End If
        Return dt
    End Function
    ...
    ''' </summary>
    Public Sub WriteDataTable(ByVal dt As DataTable)
        Using fs As New FileStream("Table.dat", FileMode.Create)
            Dim bf As New BinaryFormatter()
            bf.Serialize(fs, dt)
        End Using
