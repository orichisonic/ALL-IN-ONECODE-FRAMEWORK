=============================================================================
        CLASS LIBRARY : VBShellExtCopyHookHandler Project Overview
=============================================================================

/////////////////////////////////////////////////////////////////////////////
Summary:

The VB.NET code sample demonstrates creating a Shell copy hook handler with 
.NET Framework 4.
...
on the machine. VBShellExtCopyHookHandler is such a managed shell extension 
...
The example copy hook handler has the class ID (CLSID): 
    {FE289D2A-8D8D-4927-8069-44852CBF88F5}
...
the folder that contains the build result VBShellExtCopyHookHandler.dll and 
enter the command:

    Regasm.exe VBShellExtCopyHookHandler.dll /codebase
...
enter the command:
    ...
    Regasm.exe VBShellExtCopyHookHandler.dll /unregister
...
Install VBShellExtCopyHookHandlerSetup(x86).msi, the output of the 
VBShellExtCopyHookHandlerSetup(x86) setup project, on a x86 operating 
system. If the target operating system is x64, install 
VBShellExtCopyHookHandlerSetup(x64).msi outputted by the 
VBShellExtCopyHookHandlerSetup(x64) setup project.
...
Uninstall VBShellExtCopyHookHandlerSetup(x86).msi, the output of the 
VBShellExtCopyHookHandlerSetup(x86) setup project, on a x86 operating 
...
you will get a DLL: VBShellExtCopyHookHandler.dll. Run 'Visual Studio Command 
...
build result VBShellExtCopyHookHandler.dll and enter the command:
...
project named "VBShellExtCopyHookHandler". Open the project properties, and 
in the Signing page, sign the assembly with a strong name key file. 
...
ShellExtReg.RegisterShellExtFolderCopyHookHandler method in ShellExtLib.vb to 
    ...
    HKCR
    {
        NoRemove Directory
        {
            NoRemove shellex
            {
                ...
                {
                    VBShellExtCopyHookHandler = 
                        s '{FE289D2A-8D8D-4927-8069-44852CBF88F5}'
                }
            }
        }
    }
...
unregistration process. It removes the VBShellExtCopyHookHandler key under 
...
D. Deploying the copy hook handler with a setup project.
  ...
  (1) In VBShellExtCopyHookHandler, add an Installer class (named 
  ...
  VBShellExtCopyHookHandlerSetup(x86). Click OK to create the project. In the 
  ...
  In the Add Project Output Group dialog box, VBShellExtCopyHookHandler will  
  ...
  VBShellExtCopyHookHandler. This adds the custom actions that we defined in
  ProjectInstaller of VBShellExtCopyHookHandler. 
  ...
  Framework version of VBShellExtCopyHookHandler. 
  ...
  must create a new setup project (e.g. VBShellExtCopyHookHandlerSetup(x64) 
  ...
  file in the VBShellExtCopyHookHandlerSetup(x64) project folder. To 
  configure the script to run in the post-build event, you select the 
  VBShellExtCopyHookHandlerSetup(x64) project in Solution Explorer, and find 
