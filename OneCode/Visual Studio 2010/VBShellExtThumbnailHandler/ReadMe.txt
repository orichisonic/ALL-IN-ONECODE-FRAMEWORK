=============================================================================
         CLASS LIBRARY : VBShellExtThumbnailHandler Project Overview
=============================================================================

/////////////////////////////////////////////////////////////////////////////
Summary:

The .NET 4 code sample demonstrates the VB.NET implementation of a thumbnail 
...
The example thumbnail handler has the class ID (CLSID): 
    {E27243E6-6013-48CE-84EC-76D88A4F5158}
...
the folder that contains the build result VBShellExtThumbnailHandler.dll 
and enter the command:

    Regasm.exe VBShellExtThumbnailHandler.dll /codebase
...
and enter the command:
    ...
    Regasm.exe VBShellExtThumbnailHandler.dll /unregister
...
Install VBShellExtThumbnailHandlerSetup(x86).msi, the output of the 
VBShellExtThumbnailHandlerSetup(x86) setup project, on a x86 operating 
system. If the target operating system is x64, install 
VBShellExtThumbnailHandlerSetup(x64).msi outputted by the 
VBShellExtThumbnailHandlerSetup(x64) setup project.

B. Removal

Uninstall VBShellExtThumbnailHandlerSetup(x86).msi, the output of the 
VBShellExtThumbnailHandlerSetup(x86) setup project, on a x86 operating 
...
you will get a DLL: VBShellExtThumbnailHandler.dll. Run 'Visual Studio 
...
contains the build result VBShellExtThumbnailHandler.dll and enter the 
command:
...
named "VBShellExtThumbnailHandler". Open the project properties, and in the 
...
The RecipeThumbnailProvider.vb file defines the thumbnail handler. The 
        ...
        NoRemove .recipe
        {
            NoRemove shellex
            {
                {e357fccd-a995-4576-b01f-234630154e96} = 
                    s '{E27243E6-6013-48CE-84EC-76D88A4F5158}'
            }
        }
    }
...
D. Deploying the thumbnail handler with a setup project.
  ...
  (1) In VBShellExtThumbnailHandler, add an Installer class (named 
  ...
  VBShellExtThumbnailHandlerSetup(x86). Click OK to create the project. 
  ...
  In the Add Project Output Group dialog box, VBShellExtThumbnailHandler will  
  ...
  VBShellExtThumbnailHandler. This adds the custom actions that we defined 
  in ProjectInstaller of VBShellExtThumbnailHandler. 
  ...
  Framework version of VBShellExtThumbnailHandler. 
  ...
  must create a new setup project (e.g. VBShellExtThumbnailHandlerSetup(x64) 
  ...
  file in the VBShellExtThumbnailHandlerSetup(x64) project folder. To 
  configure the script to run in the post-build event, you select the 
  VBShellExtThumbnailHandlerSetup(x64) project in Solution Explorer, and 
